...
"""
import json, sys, time, re, glob, os
...
preds = sorted((p for p in pred_dir.glob('*.seglst.json') if p.name.split('.')[0].isdigit()), reverse=True)
t0 = time.time()
for i, p in enumerate(preds, 1):
    ...
    for tok, st, en in ts:
        ...
        for j, x in enumerate(recs):
            ...
            if d < bd: best, bd = j, d
        if best is not None: buckets[best].append(tok)
    out = []
    for j, x in enumerate(recs):
